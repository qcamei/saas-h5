...
export class AppUtils {
  ...
  //取重复
  public static repeat(arraySourceA: Array<any>,arraySourceB: Array<any>): Array<any> {
    arraySourceA = this.toNumber(arraySourceA);
    arraySourceB = this.toNumber(arraySourceB);

    let arrayTmp: Array<any> = new Array<any>();
    arraySourceA.forEach((item) => {
      if (AppUtils.arrayContains(arraySourceB,item)) {
        arrayTmp.push(item);
      }
    });
    return arrayTmp;
  ...
  public static toNumber(arraySource: Array<any>){
    let arrayTmp: Array<number> = new Array<number>();
    arraySource.forEach((item) => {
      if (typeof(item)=="string"){
        let itemTmp = parseInt(item);
        arrayTmp.push(itemTmp);
  ...
  public static showMask(title: string) {
    let temp: any = `
          <div class="popup" style="position: fixed; top: 50%; left: 50%; -webkit-transform: translate(-50%, -50%); -moz-transform: translate(-50%, -50%); -ms-transform: translate(-50%, -50%); transform: translate(-50%, -50%); background: #fff; z-index: 9999; width: 450px; -moz-border-radius: 8px; border-radius: 8px; text-align: center; background: none;">
          ...
          <div class="mask" style="position: fixed; left: 0; top: 0; height: 100%; width: 100%; background: #000; opacity: .4; z-index: 9998;"></div>
        `;
