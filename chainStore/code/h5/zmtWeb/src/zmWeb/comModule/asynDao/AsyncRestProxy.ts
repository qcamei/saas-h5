...
class HttpUtils {
  ...
  public downloadFile(urlP: string):Promise<Blob> {
    let headers = new HttpHeaders().set(SessionUtil.HEADER_ACCESS_TOKEN_NAME,SessionUtil.getInstance().getAccessToken());
    return new Promise<Blob>(resolve => {
      this.http.get(urlP,{headers: headers,observe: 'response',responseType: 'blob'}).subscribe((data:HttpResponse<Blob>) =>{
        let blobData:Blob = null;
        if(!AppUtils.isNullObj(data) && !AppUtils.isNullObj(data.body)){
          blobData = data.body
        }
        resolve(blobData);
      },error =>{
  ...
  private getHeaders():HttpHeaders{
    ...
    if(!AppUtils.isNullOrWhiteSpace(SessionUtil.getInstance().getBossId()) && !AppUtils.isNullOrWhiteSpace(SessionUtil.getInstance().getStoreId())){
      let validateInfo = ValidateInfo.newInstance(SessionUtil.getInstance().getBossId(),SessionUtil.getInstance().getStoreId());
