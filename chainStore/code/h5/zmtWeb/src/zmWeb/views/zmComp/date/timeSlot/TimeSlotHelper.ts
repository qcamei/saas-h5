...
import {ZmTime, ZmTimeData} from "../ZmTime";
...
export class TimeSlotHelper {
  ...
  public static toDate(timeStamp: number): Date {
    ...
    // console.log("before timestamp ",date.getTime());
    // date.setTime(timeStamp);
    // console.log("after  ",date);
    // console.log("after timestamp ",date.getTime());
   ...
   * @returns {DateWrap}
   */
  public static getDateWrapByDate(date: Date): DateWrap {
    return new DateWrap(date.getFullYear(), date.getMonth() + 1, date.getDate());
  }

  /**
   * 根据 date 获取 ZmTimeData
   * @param {Date} date
   * @returns {ZmTimeData}
   */
  public static getZmTimeDataByDate(date: Date): ZmTimeData {
    return new ZmTimeData(date.getHours(), date.getMinutes() + 1, date.getSeconds());
  }
  ...
  /**
   * 根据 DateWrap 和 ZmTimeData 获取时间戳
   * @param {DateWrap} dateWrap
   * @param {ZmTimeData} zmTimeDate
   * @returns {number}
   */
  public static getTimeStampByDateTime(dateWrap: DateWrap, zmTimeDate: ZmTimeData): number {
    let date: Date = new Date(dateWrap.year, dateWrap.month - 1, dateWrap.day, zmTimeDate.hour, zmTimeDate.minute, zmTimeDate.second);
    return date.getTime();
  }
}
