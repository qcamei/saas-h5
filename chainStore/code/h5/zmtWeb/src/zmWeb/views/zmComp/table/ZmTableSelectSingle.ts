...
@Component({
  selector:'zm-table-select-single',
  template: `
                ...
                <table class="zmTable">
                    ...
                    <tbody fusePerfectScrollbar>
                        <tr class="pos-r" *ngFor="let item of viewData.itemWraperList;" (click)="onClick(item)" [class.itemActiveClass]="item.hover || item.selected">
                            ...
                            <td style="width:50px;">
                              <span *ngIf="! item.selected">{{item.index+1}}</span>
                              <img src="assets/images/selectItem.png" *ngIf="item.selected" >
                            </td>
                            <ng-container *ngTemplateOutlet="tbodyTemplate;context:item"></ng-container>
                        </tr>
                        
                        <div *ngIf="viewData.itemWraperList.length==0" style=" height:50px;margin-top:170px;text-align: center">没有数据</div>
                    </tbody>
                </table>
            `,
