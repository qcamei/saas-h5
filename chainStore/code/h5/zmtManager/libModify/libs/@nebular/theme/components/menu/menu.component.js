...
var NbMenuComponent = (function() {
  ...
  NbMenuComponent.prototype.ngOnInit = function() {
    ...
    for (var i = 0; i < _a.length; i++) {
        if (_a[i].selected) {
          _a[i].img = _a[i].img + '-on';
        }
    }
  };
  NbMenuComponent.prototype.onHoverItem = function(item,e) {
    this.menuInternalService.itemHover(item, this.tag);
    //高亮左边的icon
    // for(let i = 0;i<this.items.length;i++){
    //   if(this.items[i].title === item.title){
    //       this.items[i].img = this.items[i].img.indexOf('-on') > -1 ? this.items[i].img: this.items[i].img + '-on';
    //   }else if(!this.items[i].expanded){
    //      this.items[i].img =this.items[i].img.indexOf('-on') > -1 ? this.items[i].img.slice(0, -3) : this.items[i].img;
    //   }
    // }
  };
  NbMenuComponent.prototype.onToggleSubMenu = function(item) {
    ...
    //       this.items[i].img = this.items[i].img + '-on';
    //     }
    //   } else {
    ...
    for (var i = 0; i < this.items.length; i++) {
      this.items[i].img = this.items[i].img.indexOf('-on') > -1 ? this.items[i].img.slice(0, -3) : this.items[i].img;
      if (this.items[i].title === item.title&&!item.expanded) {
        this.items[i].img = this.items[i].img + '-on';
      }
    }
  ...
  NbMenuComponent.prototype.onSelectItem = function(item) {
    ...
    for (var i = 0; i < this.items.length; i++) {
      this.items[i].img = this.items[i].img.indexOf('-on') > -1 ? this.items[i].img.slice(0, -3) : this.items[i].img;
      if (this.items[i].title === item.parent.title) {
        this.items[i].img = this.items[i].img + '-on';
      }
    }
  };
