...
/**
 *
 *  <zm_input_pwd [(zmValue)]="viewData.formData.password" [(zmPass)]="viewData.formData.passwordPass"
 (valueChecked)="viewData.formData.check()"  ></zm_input_pwd>
 */

@Component({
  selector:'zm_input_pwd',
  template: `
                <div class="disFlex align-center">
                  <label class="c-input-label">密码</label>
                  <input type="password" placeholder="请输入6-16位密码数字或字母" class="c-input"  [(ngModel)] = "zmValue" [ngClass]="{'form-valid-error':active && !zmPass}" (blur)="check()"/>
                </div>
