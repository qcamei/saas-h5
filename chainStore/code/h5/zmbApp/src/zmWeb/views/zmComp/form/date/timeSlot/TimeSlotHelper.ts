...
export class TimeSlotHelper {
  ...
  public static getTimeSlotByEnum(timeSlotEnum: TimeSlotEnum): TimeSlot {
    ...
    let timeSlot: TimeSlot = new TimeSlot(minTimeStamp, maxTimeStamp,timeSlotEnum);
    return timeSlot;
  }
  ...
  public static formatTimeSlot(timeSlot: TimeSlot): string {
    let minDate = this.toDate(timeSlot.getMinTime());
    let maxDate = this.toDate(timeSlot.getMaxTime());
    if (AppUtils.formatDate(minDate, "yyyy-MM-dd") == AppUtils.formatDate(maxDate, "yyyy-MM-dd")) {
      return AppUtils.formatDate(minDate, "yyyy-MM-dd");
    } else {
      return AppUtils.formatDate(minDate, "yyyy-MM-dd") + " - " + AppUtils.formatDate(maxDate, "yyyy-MM-dd");
    }
  }
