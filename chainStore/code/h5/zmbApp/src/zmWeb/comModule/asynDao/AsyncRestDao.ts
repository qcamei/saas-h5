...
export abstract class AsyncRestDao<T> {
  ...
  private getTargetPage(restResp: RestResp): PageResp {
  ...
  public findListWithReqParamAndPageParam(findPath, reqMap, pageItemCount, pageNo): Promise<Array<T>> {
    let uriPattern = "{0}/{1}/{2}?pageItemCount={3}&pageNo={4}&{5}";
    let reqParam = reqMap.toReqParam();
  ...
  public findPageWithReqParam(findPath:string, reqMap:ReqMap, pageNo:number):Promise<PageResp>{
    ...
    return new Promise<PageResp>(resolve => {
      this.getRestProxy().list(uri).then(
        function (restResp) {
          let targetPageTmp: PageResp = daoTmp.getTargetPage(restResp);
