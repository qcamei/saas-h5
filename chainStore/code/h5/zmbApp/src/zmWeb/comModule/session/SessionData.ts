...
export class CurrentStore{
  ...
  private _joinStoreImg:string; //加入店铺二维码
  private _acodeImg:string; //小程序二维码
  ...
  public static newInstance(store:Store,userPermData?:UserPermData,index?:number):CurrentStore{
    ...
    currentStore._bossId = store.bossId.toString();
    ...
    currentStore._joinStoreImg = store.joinStoreImg;
    currentStore._acodeImg = store.acodeImg;
    return currentStore;
  }
  ...
  public toStore():Store{
    let store = new Store();
    store.id = this._storeId;
    store.name = this._storeName;
    store.bossId = this._bossId;
    store.joinStoreImg = this._joinStoreImg;
    store.acodeImg = this._acodeImg;
    return store;
  }
...
export class UserPermData{
  ...
  finalPermSet:Array<number> = []; //最终的权限枚举集合
}
