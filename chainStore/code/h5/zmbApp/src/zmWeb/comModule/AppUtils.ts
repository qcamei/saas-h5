...
export class AppUtils {
   ...
   * @returns {number}
   */
  public static getMinTimeInday(date: Date): number {
    //当天 最小的时间戳
   ...
   * @returns {number}
   */
  public static getMaxTimeInday(date: Date): number {
    //当天 最大的时间戳
  ...
  public static isEmpty<T>(array: Array<T>): boolean{
    if(this.isNullObj(array)|| array.length == 0)return true;
    return false;
  }

  public static toNum(num: number): number{
   ...
   * @returns {Promise<T>}
   */
  public static getPromise<T>(t: T): Promise<T>{
