...
export abstract class AbsDetailDataHolder<T extends IntfDetailData>{
  ...
  public async getData(targetId:string, chainId:string):Promise<T> {
    let data:T = null;
    let version = await this.getDataVersion(chainId);
    data = DataDetailCacheMgr.getInstance().getData(this.getDataVersionEnum(),targetId,version);
    if (data == null) {
      data = await this.getMgr().get(chainId,targetId);
      if (data != null) {
  ...
  //获取店铺相关域的详情版本信息
  protected abstract getDataVersion(chainId:string):Promise<number>;
