...
export class FinancialStatisticsViewData {
  ...
  financeReports: Array<FinanceReport>;
  financeReport: FinanceReport;//列表合计后的财务统计
  ...
  public static newInstance(): FinancialStatisticsViewData {
    ...
    // target.queryForm.storeIds = SessionUtil.getInstance().getStoreId();
  ...
  public buildViewData() {
    ...
    if (!this.financeReports)
      return;
    this.financeReport = new FinanceReport();
    this.financeReports.map((financeReport: FinanceReport) => {
      this.financeReport.add(financeReport);
    });
