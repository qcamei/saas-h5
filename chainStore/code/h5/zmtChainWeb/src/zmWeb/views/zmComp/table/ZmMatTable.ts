...
/**
 ...
 <a class="zmCurHand" style="margin-right: 5px;" *ngIf="item.status == 1" (click)="turnToConsume(item.id)">转为收银</a>
 ...
 <zm-mat-table  [tdTemplateList]="[tdA,tdB,tdC,tdD,tdE]" [thNameList]="['预约时间','预约客户','接受状态','预约渠道','操作']" [itemList]="viewData.appointmentList"></zm-mat-table>


 //头部可排序
 <ng-template #thA>头像</ng-template>
 <ng-template #thB>客户姓名</ng-template>
 <ng-template #thC>
...
@Component({
  ...
  template: `
          ...
          <table mat-table [dataSource]="itemList" class="mat-elevation-z8 zmFullWidth" *ngIf="thTemplateList && thTemplateList.length > 0">
