 ...
 */
@Component({
  selector:'clerk-mat-table-checkbox',
  template: `
      
          <table  mat-table [dataSource]="itemList" class="mat-elevation-z8 zmFullWidth" >
            ...
            <ng-container matColumnDef="select">
              ...
              <td mat-cell *matCellDef="let row">
                <mat-checkbox *ngIf="row.crossClerk == 0" (click)="$event.stopPropagation()"
...
export class ClerkMatTableCheckbox implements OnInit,OnDestroy {
  ...
  isAllSelected() {
    ...
    let itemListTmp = this._itemList.filter((item)=>{if(item.crossClerk == CrossClerkEnum.True){return item}});
    const numRows = itemListTmp.length;
  ...
  allToggle() {
    ...
    if(this.isAllSelected()){
      ...
      this._itemList.forEach(item => {
        if(item.crossClerk == CrossClerkEnum.True){
