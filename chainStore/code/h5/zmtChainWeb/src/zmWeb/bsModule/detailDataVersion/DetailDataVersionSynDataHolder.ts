...
export class DetailDataVersionSynDataHolder {
  ...
  public getData(chainId: string):Promise<DetailDataVersion> {
    var dataHolder = this;
    return new Promise<DetailDataVersion>(resolve => {
      let target: DetailDataVersion = DataSynCtrl.Instance.get(DetailDataVersion, this.synType, chainId);
      if (target == null) {
        this.detailDataVersionMgr.get(chainId).then(
