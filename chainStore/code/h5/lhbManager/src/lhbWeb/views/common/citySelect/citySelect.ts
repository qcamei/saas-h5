...
@Component({
  ...
  template:
  `
	<div class="disFlex pd-l-0 pd-r-0">
	    <select style="font-size: 1rem;height: 52px;" class="form-control flex" [ngModel]="province_i" (ngModelChange)="initProvince($event)">
	    		<option value="-1">-请选择省-</option>
	    		<option *ngFor="let province of province; let i = index;" [value] = "i">{{province.name}}</option>
	    </select>
	    <select style="font-size: 1rem;height: 52px;" class="form-control flex mg-l-10 mg-r-10" [ngModel]="city_i" (ngModelChange)="initCity($event)">
				<option value="-1">- 请选择市 -</option>
				<option *ngFor="let city of city; let i = index;" [value] = "i">{{city.name}}</option>
	    </select>
	    <select style="font-size: 1rem;height: 52px;" class="form-control flex" [ngModel]="district_i" (ngModelChange)="initDistrict($event)">
